...
void ScrubStack::scrub_status(Formatter *f) {
  ...
  if (state == STATE_IDLE) {
    if (scrubbing_map.empty())
      ...
      ss << state << " (waiting for more scrubs)";
...
void ScrubStack::advance_scrub_status()
{
  ...
  for (auto it = scrubbing_map.begin(); it != scrubbing_map.end(); ) {
	...
	any_repaired = true;
      ...
      // format.
      if (up_max == 0 && !mds->is_cluster_degraded() &&
	  MDS_INO_IS_BASE(header->get_origin()) &&
	  header->get_recursive() && header->get_force() &&
	  header->get_repair()) {
	mds->mark_base_recursively_scrubbed(header->get_origin());
      }
